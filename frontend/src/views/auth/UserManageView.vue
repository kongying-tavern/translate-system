...
const users = ref<any[]>([])
const createVisible = ref(false); const createForm = reactive({ username: '', email: '', password: '', role: 'member' })
const pwdVisible = ref(false); const pwdTarget = ref<any>(null); const pwdForm = reactive({ password: '', confirmPassword: '' })
...
async function onChangeRole(row: User, newRole: string) {
  try { await updateUserRole(row.id, newRole); row.role = newRole; ElMessage.success('已更新') }
...
async function handleCreate() {
  ...
  try { const { data: res } = await createUser({ ...createForm }); users.value.push(res.data); createVisible.value = false; ElMessage.success('已添加') }
  catch (e: unknown) { ElMessage.error((e as { response?: { data?: { message?: string } } }).response?.data?.message || '失败') }
}
async function handleDelete(row: User) {
  try { await ElMessageBox.confirm(`确定删除 ${row.username} 吗？`, '确认', { type: 'warning' }) }
  catch { return }
  try { await deleteUser(row.id); users.value = users.value.filter(u => u.id !== row.id); ElMessage.success('已删除') }
  catch (e: unknown) { ElMessage.error((e as { response?: { data?: { message?: string } } }).response?.data?.message || '失败') }
}
...
async function handlePwdSave() {
  ...
  try { await changePassword(pwdTarget.value.id, pwdForm.password); pwdVisible.value = false; ElMessage.success('已修改') }
  catch (e: unknown) { ElMessage.error((e as { response?: { data?: { message?: string } } }).response?.data?.message || '失败') }
}
</script>
