<template>
  <div>
    ...
    <el-form :model="form" :rules="rules" ref="formRef" label-width="100px" style="max-width:600px">
      ...
      <el-form-item label="项目标识" prop="code"><el-input v-model="form.code" placeholder="英文标识，如 my-project" /></el-form-item>
...
async function handleCreate() {
  loading.value = true
  try { const p = await store.create(form.name, form.code, form.description, form.sourceLanguage); ElMessage.success('项目创建成功'); router.push('/projects/' + p.id) } catch { ElMessage.error('创建失败') }
