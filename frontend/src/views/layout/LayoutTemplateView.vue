    ...
    <BasePageHeader title="布局管理">
      <BaseTabs v-model="activeTab" :tabs="[{ key: 'templates', label: '模板' }, { key: 'configs', label: '配置' }]" style="margin-top:4px">
        <template #tab-templates>
          <BaseButton type="primary" style="margin-bottom:16px" @click="$router.push(`/projects/${encPathParam(projectSlug)}/layouts/templates/new/edit`)">
            新建模板
          </BaseButton>
          <BaseTable :data="templates" :columns="templateColumns" stripe />
        </template>
        <template #tab-configs>
          <BaseButton type="primary" style="margin-bottom:16px" @click="$router.push(`/projects/${encPathParam(projectSlug)}/layouts/configs/new/edit`)">
            新建配置
          </BaseButton>
