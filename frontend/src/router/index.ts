...
function hasRoutePermission(to: RouteLocationGeneric): boolean {
  ...
  const slug = decPathParam(to.params.projectSlug as string | undefined)
    ...
    },
    {
      ...
      children: [
        ...
        { path: 'projects/:projectSlug/edit', name: 'ProjectEdit', component: () => import('@/views/project/ProjectCreateView.vue'), meta: { title: '编辑项目', perm: 'sys:super_admin' } },
        {
          ...
          children: [
            { path: '', redirect: (to: RouteLocationGeneric) => `/projects/${encPathParam(decPathParam(to.params.projectSlug as string) as string)}/translations` },
