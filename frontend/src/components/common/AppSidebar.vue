<template>
  <div class="sidebar">
    ...
    <el-menu :default-active="route.path" router background-color="#1d1e2c" text-color="#bfcbd9" active-text-color="#409eff" class="sidebar-menu">
      <el-menu-item v-if="auth.role === 'super_admin' || auth.role === 'admin'" index="/users"><el-icon><User /></el-icon><span>用户管理</span></el-menu-item>
      <template v-if="projectSlug">
        <el-sub-menu index="projects">
          ...
          <el-menu-item v-if="auth.role !== 'member'" :index="`/projects/${projectSlug}/imports`"><el-icon><Upload /></el-icon><span>导入模板</span></el-menu-item>
...
import { User, Monitor, Avatar, Document, Collection, Download, Upload } from '@element-plus/icons-vue'
