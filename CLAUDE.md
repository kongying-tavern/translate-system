...
- **导入锁（共享 composable）**：`frontend/src/hooks/useImportStatus.ts` 封装 `GET /projects/{slug}/imports/status` 轮询（频率随锁状态切换：进行中 2 秒 / 空闲 30 秒，slug 变更自动重置、组件卸载自动停止），向所有受导入影响的页面暴露 `locked` / `lockType` / `locker` / `progressText` 等响应式状态。`progressText` 按阶段给出进度文案：解析阶段「解析中 X 条目 / Y 字段」；写入阶段条目模式用键维度、翻译模式用字段维度显示「进度 X / Y（Z%）」（已处理 = 已写入 + 已跳过）；done 为「写入完成」。`status` 接口对任意项目成员开放（仅 `@Security('auth')` + `assertProjectAccess`），锁状态对**所有角色、跨标签页**实时生效，防止页面编辑与后台导入并发写入冲突。
