...
- **编辑页导入锁**（`ProjectCreateView.vue`，路由 `/projects/:slug/edit` 复用创建页 isEdit 模式；`ProjectSettingsView.vue` 未挂路由属遗留文件勿再改它）：锁定时展示警告条并禁用整个表单与删除按钮（handleDelete/handleSubmit 有守卫）；提交中 loading 同样锁定全表单；新建模式无项目不涉及。
