...
- **写入性能优化**：`ImportsController` 的 `importKeys` / `applyTranslations` 写入阶段（每批 `IMPORT_BATCH=1000`）已全部走原生批量/参数化 SQL，不再有逐条事务——A. 译文/原文值用 `bulkWriteTranslationValues` 单条 `INSERT ... ON CONFLICT (key_id, language_code) DO UPDATE`（`$executeRaw`+`Prisma.join`/`Prisma.sql`，参数化防注入，同 `(keyId,lang)` 经 Map 去重即优化 E）批量 upsert，替代逐条 `translationValue.update`；B. Key 的 `context`/`tags` 用 `bulkUpdateKeyMeta` 单条 `UPDATE ... FROM (VALUES ...)` 一次完成（tags 为 `string[]`，参数化 + `::text[]` 交由驱动转义，无注入风险）；C. B 与 A 合并进同一个 `$transaction` 提交，减少一次往返；D. 新建 Key 用 `createManyAndReturn` 直接拿回 `{id,key}`，省去此前 `createMany` 后的 `findMany` 回查；E. `toWrite` 同 `(keyId,lang)` 经 Map 去重取末值；F. `applyTranslations` 的值预载由 `keyId IN ∧ lang IN` 改为 `(key_id, language_code) 元组 IN` 精确匹配（参数化），避免笛卡尔积回查。SSE 推送（`getImportStatusStream`）导入进行中按 250ms 节流（解锁/结束态恒推），且节流判断前置到 `buildImportStatusRow` 之前，省去被节流时的建行 + JSON 序列化开销。改动写入逻辑时勿退回逐条事务。
