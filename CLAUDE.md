...
- **写入性能优化**：`ImportsController` 的 `importKeys` / `applyTranslations` 写入阶段（每批 `IMPORT_BATCH=1000`）已做以下优化——① 译文/原文值用原生单条 `INSERT ... ON CONFLICT (key_id, language_code) DO UPDATE`（经 `bulkWriteTranslationValues` 的 `$executeRaw` + `Prisma.join`/`Prisma.sql`，参数化防注入）批量 upsert，替代逐条 `translationValue.update` 事务；② Key 的 `context`/`tags` 更新收集后按 key 去重，`$transaction` 批量执行；③ 本导入内 `key→id` 映射跨批 `Map` 缓存，避免每批 `findMany` 重复查 key；④ SSE 推送（`getImportStatusStream`）在导入进行中按 250ms 节流（解锁/结束态恒推），避免大批量逐批推送刷爆前端。改动写入逻辑时勿退回逐条事务。
