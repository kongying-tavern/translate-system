param(
    ...
    [Alias("o")]
    [Parameter(HelpMessage = "覆盖已有译文（默认不覆盖）")]
    [switch]$Overwrite,

    [Alias("n")]
    [Parameter(HelpMessage = "不自动补全新条目（默认自动创建）")]
    [switch]$NoAutoCreate
)
...
if (-not $FormatType) { $missing += "FormatType" }
if (-not $Language) { $missing += "Language" }
...
$body = @{
    ...
    autoCreate   = (-not $NoAutoCreate.IsPresent)
} | ConvertTo-Json -Compress

Write-Host "正在导入翻译到项目 $ProjectSlug（语言: $Language，格式: $FormatType）..." -ForegroundColor Cyan
...
try {
    ...
    $rawJson = $wc.UploadString("$Endpoint/api/v1/apikey/projects/$ProjectSlug/imports/translations", "POST", $body)
