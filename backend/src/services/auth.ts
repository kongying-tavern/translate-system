...
export async function login(account: string, password: string) {
  ...
  if (!user)
    throw { code: 1001, message: '用户名/邮箱或密码错误' }
...
export async function register(username: string, email: string, password: string) {
  ...
  if (existingEmail)
    throw { code: 1004, message: '邮箱已注册' }
  const existingUsername = await prisma.user.findUnique({ where: { username } })
  if (existingUsername)
    throw { code: 1004, message: '用户名已存在' }
...
export async function refresh(refreshToken: string) {
  ...
  if (!stored)
    throw { code: 1001, message: 'invalid or expired refresh token' }
  await prisma.refreshToken.update({ where: { id: stored.id }, data: { revoked: true } })
  const user = await prisma.user.findUnique({ where: { id: stored.userId } })
  if (!user)
    throw { code: 1003, message: 'user not found' }
...
export async function updateUserRole(operatorId: string, targetId: string, newRole: string) {
  ...
  if (!canManage(operator?.role, target.role))
    throw { code: 1002, message: '没有权限管理此用户' }
  if (ROLE_LEVEL[newRole] > (ROLE_LEVEL[operator?.role || 'member'] || 0))
    throw { code: 1002, message: '不能设置高于自己的角色' }
...
export async function createUser(username: string, email: string, password: string, role: string, operatorRole?: string) {
  ...
  if (operatorRole === 'admin' && role !== 'member')
    throw { code: 1002, message: '系统管理员只能创建成员' }
  const passwordHash = await bcrypt.hash(password, 10)
...
export async function deleteUser(operatorId: string, targetId: string) {
  ...
  if (targetId === operatorId)
    throw { code: 1004, message: '不能删除自己' }
  if (!canManage(operator?.role, target.role))
    throw { code: 1002, message: '没有权限删除此用户' }
...
export async function changeOwnPassword(userId: string, oldPassword: string, newPassword: string) {
  ...
  if (!await bcrypt.compare(oldPassword, user.passwordHash))
    throw { code: 1001, message: '当前密码错误' }
...
export async function changeUserPassword(operatorId: string, targetId: string, newPassword: string) {
  ...
  if (!target)
    throw { code: 1003, message: '用户不存在' }
  if (!canManage(operator?.role, target.role))
    throw { code: 1002, message: '不能修改此用户密码' }
  const passwordHash = await bcrypt.hash(newPassword, 10)
