...
export function exportCSV(translations: FlatTranslation[], langs: string[], config?: Record<string, unknown>) {
  const rows: Record<string, { langs: Record<string, string> }> = {}
  for (const t of translations) {
    if (!rows[t.translationKey])
      rows[t.translationKey] = { langs: {} }
  ...
  const header = [csvEscape('key'), ...headerNames.map(csvEscape)].join(',')
  const lines = [header]
  for (const [key, row] of Object.entries(rows)) {
    lines.push([csvEscape(key), ...langs.map(l => csvEscape(row.langs[l] || ''))].join(','))
  }
  return lines.join('\n')
}
