...
authRoutes.post('/register', async (req, res) => {
  try {
    ...
    if (!username || !email || !password)
      return error(res, ErrCode.InvalidParams, 'missing required fields')
...
authRoutes.post('/login', async (req, res) => {
  try {
    ...
    if (!loginAccount || !password)
      return error(res, ErrCode.InvalidParams, '请输入用户名/邮箱和密码')
...
authRoutes.post('/refresh', async (req, res) => {
  try {
    ...
    if (!refreshToken)
      return error(res, ErrCode.InvalidParams, 'missing refreshToken')
...
authRoutes.put('/me/password', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!oldPassword || !newPassword)
      return error(res, ErrCode.InvalidParams, '缺少密码')
